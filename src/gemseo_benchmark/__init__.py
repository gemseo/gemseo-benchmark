...
The :mod:`~gemseo-benchmark.problems` sub-package handles the benchmarking problems and groups of problems.
...
* the target values of problem, necessary to compute :ref:`data profiles <data-profiles>`.


Performance history
-------------------

The :mod:`gemseo-benchmark.results` sub-package manages the results of the benchmarked algorithms.
...
#. This :class:`.HistoryItem` is appended to a :class:`PerformanceHistory`,
   ...
   The index of the sequence is understood as the 0-based number of functions evaluations.
...
The :mod:`~gemseo_benchmark.report` sub-package manages the automatic generation of a benchmarking report.

The :class:`Report` produces `reStructuredText (reST) <https://docutils.sourceforge.io/rst.html>`_ files describing:
