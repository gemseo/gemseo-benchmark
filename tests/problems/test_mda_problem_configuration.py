...
def test_default_starting_point(mda_variable_space) -> None:
    """Check that the default starting point is properly set."""
    check_default_starting_point(
        MDAProblemConfiguration("Linear MDA", mda_create_problem, mda_variable_space),
        mda_variable_space,
    )


def test_inconsistent_starting_points(mda_variable_space) -> None:
    """Check initialization with starting points of inadequate size."""
    check_inconsistent_starting_points(
        MDAProblemConfiguration, mda_create_problem, mda_variable_space
    )


def test_undefined_target_values(mda_variable_space) -> None:
    """Check undefined target values."""
    check_undefined_target_values(
        MDAProblemConfiguration("SSBJ MDA", mda_create_problem, mda_variable_space)
    )


def test_generate_default_starting_points(mda_variable_space) -> None:
...
def test_generate_starting_points(mda_variable_space) -> None:
...
def test_description(
    mda_problem_configuration, mda_variable_space, input_description, actual_description
) -> None:
...
def test_save_starting_points(tmp_path, mda_variable_space) -> None:
    """Check the saving of starting points."""
    check_starting_points_saving(
        tmp_path, MDAProblemConfiguration, mda_create_problem, mda_variable_space
    )


def test_load_starting_points(tmp_path, mda_variable_space) -> None:
...
def test_variable_space(mda_problem_configuration, mda_variable_space) -> None:
    """Check the variable space."""
    check_variable_space(mda_problem_configuration, mda_variable_space)
